...
import { Calendar, ArrowRight, X, Share2, Bookmark, Clock, User } from "lucide-react";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {newsItems.map((item, i) => (
            <motion.article
              ...
              className="group bg-white rounded-2xl border border-gray-100/50 overflow-hidden hover:shadow-xl hover:shadow-black/5 transition-all duration-500 cursor-pointer"
            >
              ...
              <div className="p-5">
                ...
                <h3 className="text-sm font-semibold text-secondary leading-snug mb-2 line-clamp-2 group-hover:text-primary transition-colors">
                  {item.title}
                </h3>
      ...
      {selectedArticle && (
        ...
        >
          <motion.div
            ...
            className="bg-white rounded-3xl max-w-xl w-full shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
          ...
          >
            <div className="relative h-56 bg-gray-100">
              ...
              <button
                onClick={() => setSelectedArticle(null)}
                className="absolute top-4 right-4 w-8 h-8 rounded-xl bg-white/90 backdrop-blur-sm flex items-center justify-center text-gray-400 hover:text-gray-600"
              >
                <X size={16} />
              </button>
              ...
              {articleContents[selectedArticle.id]?.tags && (
                <div className="flex flex-wrap gap-2 mb-6">
                  {articleContents[selectedArticle.id].tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 rounded-full bg-gray-100 text-xs text-gray-500 font-medium"
                    >
                      #{tag}
                    </span>
                  ))}
